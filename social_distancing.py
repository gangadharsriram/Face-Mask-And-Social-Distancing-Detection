...
detector = cv2.dnn.readNetFromCaffe(prototxt="MobileNetSSD_deploy.prototxt", 
...
def non_max_suppression_fast(boxes, overlapThresh):
    try:
        if len(boxes) == 0:
            ...
            #current_time = now.strftime("%I:%M:%S") current_time
            current_time = datetime.datetime.now()
            print("no person detected",current_time)
...
path=r"input video\test_video.mp4"
